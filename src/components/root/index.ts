export default ($targetElement: Element, template: string): void => {
  const realDom = $targetElement;
  const virtualDom = realDom.cloneNode(true) as Element;
...
const getMaxLength = (target1 = 0, target2 = 1): number => {
  return Math.max(target1, target2);
};

const getThisDomChilds = (target: Element): [Element[], number] => {
  const targetChildrens = Array.from(target.children);
  return [targetChildrens, targetChildrens.length];
};

const isChangedNode = (node1: Element, node2: Element): boolean => {
...
const isNotEquals = (target1: NamedNodeMap, target2: NamedNodeMap): boolean => {
  return target1 !== target2;
};

const isNotEqualsLastContent = (node1: Element, node2: Element): boolean => {
...
const isDifferenceAttrtibue = (node1: Element, node2: Element) => {
  return Array.from(node1.attributes).find((attribute: Attr): boolean => {
    const name: string = attribute.name;
    const real: string | null = node1.getAttribute(name);
    const virtual: string | null = node2.getAttribute(name);
    return real !== virtual;
  });
};

const isOnlyExistReal = (node1: Element, node2: Element): boolean => {
  return node1 && !node2;
};

const isOnlyExistVirtual = (node1: Element, node2: Element): boolean => {
  return !node1 && node2;
};
